	...
	If($aduser -eq $null)
	{
        ...
        IF([bool](get-aduser -Filter{SamAccountName -eq $initusername} -ErrorAction SilentlyContinue) -eq $true)
        {
            ...
            ELSEIF([bool](get-aduser -Filter{SamAccountName -eq $secondusername} -ErrorAction SilentlyContinue) -eq $false)
            {
                ...
                New-ADUser -SamAccountName $secondusername -UserPrincipalName ($secondusername+"@belltechlogix.com") -Name ($user."First Name"+" "+$user."Last Name" ) -Surname $user."Last Name" -GivenName $user."First Name" -Initials $user.'Middle Initial' -EmployeeNumber $user."Associate ID" -Department ($deptlookup[$user."Home Department Code".trim().trimstart('0')]) -Manager $manager.SamAccountName -Title $user.'Job Title Description' -OfficePhone $user.'Work Contact: Work Phone' -MobilePhone $user.'Personal Contact: Personal Mobile' -path "OU=\#\#Automation_Purgatory,DC=btl,DC=bellind,DC=net" -Enabled 1 -PasswordNotRequired 1 -ErrorAction Continue -WhatIf|Add-Content $log			
        ...
        ELSEIF([bool](get-aduser -Filter{SamAccountName -eq $initusername} -ErrorAction SilentlyContinue) -eq $false)
        {
            ...
            New-ADUser -SamAccountName $initusername -UserPrincipalName ($initusername+"@belltechlogix.com") -Name ($user."First Name"+" "+$user."Last Name" ) -Surname $user."Last Name" -GivenName $user."First Name" -Initials $user.'Middle Initial' -EmployeeNumber $user."Associate ID" -Department ($deptlookup[$user."Home Department Code".trim().trimstart('0')]) -Manager $manager.SamAccountName -Title $user.'Job Title Description' -OfficePhone $user.'Work Contact: Work Phone' -MobilePhone $user.'Personal Contact: Personal Mobile' -path "OU=\#\#Automation_Purgatory,DC=btl,DC=bellind,DC=net" -Enabled 1 -PasswordNotRequired 1 -ErrorAction Continue -WhatIf|Add-Content $log
